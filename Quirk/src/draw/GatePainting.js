...
const GATE_SYMBOL_FONT = '16px Helvetica Neue';

GatePainting.paintOutline = (args) => {
    const radius = 6; // radio de las esquinas
    const ctx = args.painter.ctx;
    const r = args.rect;

    // Empieza a trazar el contorno redondeado
    ctx.save();
    ctx.beginPath();
    // Punto inicial: arriba a la izquierda, desplazado por radius
    ctx.moveTo(r.x + radius, r.y);

    // Línea superior hasta arriba a la derecha menos radius
  ...
  /*
  if (args.isInToolbox) {
    // Si quieres además la línea inferior tradicional...
    let topRight = r.topRight(),
      bottomRight = r.bottomRight(),
      bottomLeft = r.bottomLeft();
    args.painter.strokeLine(topRight, bottomRight);
    args.painter.strokeLine(bottomLeft, bottomRight);
  }
    */
};
...
GatePainting.paintBackground = (
  args,
  toolboxFillColor = "#df7af3",
  normalFillColor = "#f2f51f"
) => {
...
GatePainting.paintResizeTab = args => {
    ...
    let backColor = args.isResizeHighlighted ? "#87d9ff" : "#87d9ff";
    let foreColor = args.isResizeHighlighted ? "#33B1FF" : "gray";
 ...
 * @param {!string} toolboxFillColor
 */
GatePainting.paintLocationIndependentFrame = (
  args,
  normalFillColor = "#8de514",
  toolboxFillColor = "#8de514"
) => {
  if (args.isInToolbox) {
    GatePainting.paintBackground(args, toolboxFillColor, normalFillColor);
    GatePainting.paintOutline(args);
  ...
  let backColor = args.isHighlighted
    ? Config.HIGHLIGHTED_GATE_FILL_COLOR
    : normalFillColor;
  args.painter
    .trace((tracer) =>
      GatePainting.traceLocationIndependentOutline(args, tracer)
...
GatePainting.PERMUTATION_DRAWER = args => {
    ...
    if (args.isHighlighted ||
        ...
        GatePainting.paintBackground(args, '#33B1FF', '#F3F3F3');
        GatePainting.paintOutline(args);
        GatePainting.paintResizeTab(args);
    } else {
