...
class DisplayedCircuit {
  ...
  _groundedWireCount() {
    let pseudoCount =
      this._extraWireStartIndex !== undefined &&
      this._extraWireStartIndex !== Config.MAX_WIRE_COUNT
        ? 1
        : 0;

    let n =
      Math.max(Config.MIN_WIRE_COUNT, this.circuitDefinition.numWires) -
      pseudoCount;
  ...
  desiredWidth(forTooltip = false) {
    if (forTooltip) {
      return (
        this.opRect(this.circuitDefinition.columns.length - 1).right() +
  ...
  wireRect(wireIndex) {
    ...
    return new Rect(
      0,
      this.top + Config.WIRE_SPACING * wireIndex,
      Infinity,
      Config.WIRE_SPACING
    );
  }
  ...
  indexOfDisplayedColumnAt(x) {
    ...
    let i;
    if (
      this._compressedColumnIndex === undefined ||
      col < this._compressedColumnIndex - 0.75
    ) {
      i = Math.round(col);
  ...
  findOpHalfColumnAt(p) {
    ...
    return Math.max(
      -0.5,
      Math.round(this.toColumnSpaceCoordinate(p.x) * 2) / 2
    );
  }
  ...
  _findModificationIndex_helperColRow(hand) {
    ...
    let pos = hand.pos
      ...
      .plus(new Point(Config.GATE_RADIUS, Config.GATE_RADIUS));
  ...
  findModificationIndex(hand) {
    ...
    if (!isInsert) {
      let mustInsert =
        this.circuitDefinition.isSlotRectCoveredByGateInSameColumn(
          col,
          row,
  ...
  opRect(operationIndex) {
    ...
    if (
      this._compressedColumnIndex !== undefined &&
      operationIndex === this._compressedColumnIndex
    ) {
  ...
  gateRect(wireIndex, operationIndex, width = 1, height = 1) {
    ...
    return new Rect(
      Math.round(r.x - 0.5) + 0.5,
      Math.round(r.y - 0.5) + 0.5,
      Math.round(r.w),
      Math.round(r.h)
    );
  }
  ...
  isEqualTo(other) {
    ...
    return (
      other instanceof DisplayedCircuit &&
      this.top === other.top &&
  ...
  paint(painter, hand, stats, forTooltip = false, showWires = true) {
    ...
    for (let col = 0; col < this.circuitDefinition.columns.length; col++) {
      this._drawColumn(
        painter,
        this.circuitDefinition.columns[col],
        col,
        hand,
        stats
      );
  ...
  _drawWires(painter, showLabels, hand) {
    let drawnWireCount = Math.min(
      this.circuitDefinition.numWires,
      (this._extraWireStartIndex || Infinity) + 1
    );
    ...
    if (showLabels) {
      for (let row = 0; row < drawnWireCount; row++) {
        ...
        if (
          this._highlightedSlot === undefined &&
          hand.pos !== undefined &&
          rect.containsPoint(hand.pos)
        ) {
      ...
      painter
        .trace((trace) => {
          ...
          for (
            let col = 0;
            showLabels
              ? lastX < painter.canvas.width
              : col <= this.circuitDefinition.columns.length;
            col++
          ) {
    ...
    if (
      this._extraWireStartIndex !== undefined &&
      this.circuitDefinition.numWires === Config.MAX_WIRE_COUNT
    ) {
      painter.print(
  ...
  _highlightStatusAt(col, row, focusPosPts) {
    ...
    let isOverGate = (pos) => {
      ...
      return (
        overGate !== undefined && overGate.col === col && overGate.row === row
      );
    };
    ...
    let isOverGateResizeTab = (pos) =>
      isNotCoveredAt(pos) && resizeTabRect.containsPoint(pos);

    let isResizeHighlighted =
      gate.canChangeInSize() && seq(focusPosPts).any(isOverGateResizeTab);
    let isHighlighted =
      !isResizeHighlighted && seq(focusPosPts).any(isOverGate);
    let isResizeShowing =
      gate.canChangeInSize() && (isResizeHighlighted || isHighlighted);
  ...
  _drawGate_disabledReason(painter, col, row, gateRect, isHighlighted) {
    let isDisabledReason = this.circuitDefinition.gateAtLocIsDisabledReason(
      col,
      row
    );
  ...
  _drawColumnDragHighlight(painter, col) {
    if (
      ...
      let rect = this.gateRect(0, col, 1, this._groundedWireCount()).paddedBy(
        3
      );
  ...
  _drawColumnControlWires(painter, columnIndex) {
    ...
    if (
      this.circuitDefinition.columns[columnIndex].hasGatesWithGlobalEffects()
    ) {
      painter.ctx.save();
    ...
    for (let {
      ...
      last,
      measured,
    } of this.circuitDefinition.controlLinesRanges(columnIndex)) {
   ...
   */
  previewDrop(hand) {
    return hand.heldRow !== undefined
      ? this._previewDropMovedRow(hand)
      : hand.heldColumn !== undefined
      ? this._previewDropMovedGateColumn(hand)
      : hand.heldGate !== undefined
      ? this._previewDropMovedGate(hand)
  ...
  _previewDropMovedRow(hand) {
    ...
    let heldRowHeight = seq(hand.heldRow.gates)
      .map((g) => (g === undefined ? 1 : g.height))
      .max(1);
    handWire = Math.min(
      handWire,
      this.circuitDefinition.numWires - heldRowHeight
    );
    ...
    let newInitialStates = seq(
      this.circuitDefinition.customInitialValues.entries()
    )
  ...
  _previewDropMovedGateColumn(hand) {
    ...
    let rowShift = Math.round(
      (hand.pos.y - hand.holdOffset.y - this.top) / Config.WIRE_SPACING
    );
    let newCircuitDef = this._shiftAndSpliceColumn(
      rowShift,
      [...hand.heldColumn.gates],
      col,
      isInsert
    );
  ...
  _shiftAndSpliceColumn(rowShift, gatesOfCol, insertCol, isInsert) {
    ...
    while (
      rowShift > 0 &&
      new GateColumn(gatesOfCol).minimumRequiredWireCount() <
    ...
    let expandedCircuit = this.circuitDefinition.withWireCount(
      gatesOfCol.length
    );
  ...
  _previewDropMovedGate(hand) {
    ...
    if (
      modificationPoint.row + handRowOffset >=
      this.circuitDefinition.numWires
    ) {
      return this;
    }
    ...
    let row = Math.min(
      ...
      Math.max(0, Config.MAX_WIRE_COUNT - addedGate.height)
    );
    let newCols = seq(this.circuitDefinition.columns)
      ...
      .withTransformedItem(i, (c) =>
        c.withGatesAdded(row, new GateColumn([addedGate]))
      )
      .toArray();
    let newWireCount = Math.max(
      this._extraWireStartIndex || 0,
      Math.max(this.circuitDefinition.numWires, addedGate.height + row)
    );
  ...
  _previewResizedGate(hand) {
    ...
    let gate = this.circuitDefinition.gateInSlot(
    ...
    let newGate = seq(gate.gateFamily).minBy((g) =>
      Math.abs(g.height - (row - hand.resizingGateSlot.y + 1))
    );
    let newWireCount = Math.min(
      Config.MAX_WIRE_COUNT,
      Math.max(
        this.circuitDefinition.numWires,
    ...
    let newCircuitWithoutOverlapFix = this.circuitDefinition
      .withColumns(newCols)
      .withWireCount(newWireCount);
    let newCircuitWithOverlapFix =
    ...
    return this.withCircuit(newCircuit)
      ...
      ._withCompressedColumnIndex(
        newCircuitWithoutOverlapFix.isEqualTo(newCircuitWithOverlapFix)
          ? undefined
  ...
  _withFallbackExtraWireStartIndex(fallbackExtraWireStartIndex) {
    return this._withExtraWireStartIndex(
      this._extraWireStartIndex || fallbackExtraWireStartIndex
    );
  }
  ...
  withJustEnoughWires(hand, extraWireCount) {
    let neededWireCountForPlacement =
      hand.heldGate !== undefined ? hand.heldGate.height : 0;
    ...
    return this.withCircuit(
      this.circuitDefinition.withWireCount(clampedWireCount)
    )._withExtraWireStartIndex(
      extraWireCount === 0 ? undefined : this.circuitDefinition.numWires
    );
  }
  ...
  findGateWithButtonContaining(pos) {
    ...
    let buttonRect = GatePainting.gateButtonRect(
      this.gateRect(foundPt.row, foundPt.col, gate.width, gate.height)
    );
  ...
  tryClick(hand) {
    ...
    let clickedInitialStateWire = this.findWireWithInitialStateAreaContaining(
      hand.pos
    );
    if (clickedInitialStateWire !== undefined) {
      return this.withCircuit(
        this.circuitDefinition.withSwitchedInitialStateOn(
          clickedInitialStateWire
        )
      );
    }
  ...
  tryGrab(
    ...
    if (wholeColumn) {
      ...
      return (
        this._tryGrabWholeColumn(hand, duplicate, alt) || {
          newCircuit: this,
          newHand: hand,
        }
      );
    ...
    return (
      newCircuit._tryGrabGate(newHand, duplicate, alt) || {
        newCircuit,
        newHand,
      }
    );
  ...
  _cutRow(row) {
    ...
    return {
      newCircuit: this.circuitDefinition
        .withColumns(cols)
        .withInitialStates(newInitialStates),
      rowGates: row_gates,
  ...
  _tryGrabGate(hand, duplicate, alt) {
    ...
    let remainingGates = seq(
      this.circuitDefinition.columns[col].gates
    ).toArray();
    if (!duplicate) {
  ...
  _tryGrabResizeTab(hand) {
    ...
    for (let col = 0; col < this.circuitDefinition.columns.length; col++) {
      for (let row = 0; row < this.circuitDefinition.numWires; row++) {
        ...
        let { isResizeHighlighted } = this._highlightStatusAt(
          col,
          row,
          hand.hoverPoints()
        );
        if (isResizeHighlighted) {
          let offset = hand.pos.minus(
            this.gateRect(row + gate.height - 1, col, 1, 1).center()
          );
          return {
  ...
  _tryGrabWholeColumn(hand, duplicate, alt) {
    ...
    if (
      col < 0 ||
      col >= this.circuitDefinition.columns.length ||
      this.circuitDefinition.columns[col].isEmpty()
    ) {
      return undefined;
    }
    ...
    let holdOffset = new Point(
      0,
      this.wireIndexAt(hand.pos.y) * Config.WIRE_SPACING +
        Config.WIRE_SPACING / 2
    );
    let grabbedGates = this.circuitDefinition.columns[col];
    if (alt) {
      grabbedGates = new GateColumn(
        grabbedGates.gates.map((e) => (e === undefined ? e : e.alternate))
      );
    }
    return {
  ...
  importantWireCount() {
    return Math.max(
      this.circuitDefinition.numWires -
        (this._extraWireStartIndex === Config.MAX_WIRE_COUNT ? 0 : 1),
  ...
  _drawOutputDisplays(painter, stats, hand) {
    ...
    for (let i = 0; i < numWire; i++) {
      ...
      MathPainter.paintProbabilityBox(
        painter,
        p,
        this.gateRect(i, chanceCol),
        hand.hoverPoints()
      );
      let m = stats.qubitDensityMatrix(Infinity, i);
      if (m !== undefined) {
        paintBlochSphereDisplay(
          painter,
          m,
          this.gateRect(i, blochCol),
          hand.hoverPoints()
        );
      }
    }
  ...
  _drawOutputSuperpositionDisplay(painter, stats, hand) {
    ...
    MathPainter.paintMatrixTooltip(
      painter,
      amplitudeGrid,
      gridRect,
      hand.hoverPoints(),
      (c, r) =>
        `Amplitude of |${Util.bin(
          r * amplitudeGrid.width() + c,
          numWire
        )}⟩ (decimal ${r * amplitudeGrid.width() + c})`,
      (c, r, v) => "val:" + v.toString(new Format(false, 0, 5, ", ")),
      (c, r, v) =>
        `mag²:${(v.norm2() * 100).toFixed(4)}%, phase:${forceSign(
          (v.phase() * 180) / Math.PI
        )}°`
    );
  ...
  _outputStateAsMatrix(stats) {
    ...
    let [colWires, rowWires] = [
      Math.floor(numWire / 2),
      Math.ceil(numWire / 2),
    ];
  ...
  _rectForSuperpositionDisplay() {
    let col =
      this.clampedCircuitColCount() + EXTRA_COLS_FOR_SINGLE_QUBIT_DISPLAYS + 1;
    let numWire = this.importantWireCount();
    let [colWires, rowWires] = [
      Math.floor(numWire / 2),
      Math.ceil(numWire / 2),
  ...
  _drawHintLabels(painter, stats) {
    ...
    if (this.circuitDefinition.colIsMeasuredMask(Infinity) !== 0) {
      painter.printParagraph(
        ...
        new Rect(gridRect.right() + 3, gridRect.bottom() + 20, 100, 75),
        new Point(0.5, 0),
        "red"
      );
    }
    ...
    if (Math.abs(survivalRate - 1) > 0.01) {
      ...
      if (survivalRate < 1) {
        ...
        let rateDesc = survivalRate === 0 ? "0" : rate > 0 ? rate : "<1";
        desc = `kept: ${rateDesc}%`;
      } else {
  ...
  static fromTextDiagram(gateMap, diagramText) {
    ...
    let pts = Seq.naturals()
      ...
      .map((k) => {
        let pos = seq(lines)
          .mapWithIndex((line, row) => ({ row, col: line.indexOf(k) }))
          .filter((e) => e.col !== -1)
          .single();
...
function drawCircuitTooltip(painter, circuitDefinition, rect, showWires, time) {
  ...
  let stats = CircuitStats.withNanDataFromCircuitAtTime(
    circuitDefinition,
    time
  );
  try {
...
let _cachedColLabelDrawer = new CachablePainting(
  (numWire) => {
    ...
    let [colCount, rowCount] = [1 << colWires, 1 << rowWires];
    let total_height =
      (numWire - 1) * Config.WIRE_SPACING + Config.GATE_RADIUS * 2;
